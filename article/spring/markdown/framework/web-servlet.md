...
교차 오리진 요청(즉, 오리진 헤더가 존재하고 요청의 호스트와 다름)을 활성화하려면 몇 가지 명시적으로 선언된 CORS 구성이 있어야 한다. 일치하는 CORS 구성을 찾을 수 없으면 preflight 요청이 거부된다. 단순하고 실제적인 CORS 요청의 응답에 CORS 헤더가 추가되지 않으므로 브라우저는 이 헤더를 거부한다.
